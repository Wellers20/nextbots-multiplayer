...
import { useFBX, useAnimations, PositionalAudio, useProgress } from '@react-three/drei';
...
export const Nextbot = ({ walls }: { walls: number[][] }) => {
  ...
  // Load both FBX files
  const walkFbx = useFBX('https://raw.githubusercontent.com/Wellers20/OcenIVATel/main/Strut%20Walking.fbx');
      ...
      if (map === 'house' && houseGraph) {
        ...
        if (nodes.length > 0) {
          while (!found && attempts < 100) {
            ...
            // Don't spawn too close to player (player spawns at grid 3,0,3 in house)
  ...
  useFrame((state, delta) => {
    ...
    globalMonsterPos.copy(bot.position);
    
    setSpeed(Math.round(velocity.current.length() * 10));
    ...
    const checkLineOfSight = (start: THREE.Vector3, end: THREE.Vector3) => {
      if (map === 'house') {
        // Simple check for house: must be on same floor and no walls between
        if (Math.abs(start.y - end.y) > 2) return false;
        // Basic raycast check using grid
    ...
    // If moving between floors, allow Y movement, otherwise keep it flat for rotation
    ...
    // Only apply grid collisions if in maze
    if (map === 'maze') {
      const radius = 1.0;
      ...
      // In house, we rely on the pathfinding graph to keep us out of walls
      // But we should also prevent clipping into walls
      const radius = 1.0;
  ...
  return (
    <group ref={ref} position={[20, 0, 20]}>
      ...
      {/* Audio Effects - Mounted unconditionally to prevent Suspense mid-game */}
      <PositionalAudio
        ref={walkAudioRef}
        url="https://raw.githubusercontent.com/Wellers20/OcenIVATel/main/steps-2.mp3"
        distance={10}
        loop
        autoplay={false}
      />
...
// Preload assets to prevent mid-game loading stutters
useFBX.preload('https://raw.githubusercontent.com/Wellers20/OcenIVATel/main/Strut%20Walking.fbx');
useFBX.preload('https://raw.githubusercontent.com/Wellers20/OcenIVATel/main/Run.fbx');
useLoader.preload(THREE.AudioLoader, 'https://raw.githubusercontent.com/Wellers20/OcenIVATel/main/steps-2.mp3');
